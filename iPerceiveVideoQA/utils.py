...
def loadPreTrainedModel(model, modelPath):
    if os.path.exists(modelPath):
        ...
        logger.info('Pre-trained model loaded from:', modelPath)
    else:
        logger.error('Couldn\'t find pre-trained model at:', modelPath)
...
def printNetworkArch(net):
    logger.info('------------- iPerceive VideoQA Network Architecture -------------')
    if net:
        _printNetworkArch(net)
