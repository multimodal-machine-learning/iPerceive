...
# knobs to set TVQA paths
paths = {}
paths["data_path"] = "./data/"
paths["raw_data_path"] = "./raw_data/"
paths["indices"] = paths["data_path"]
paths["densecap_dict"] = paths["data_path"]
paths["qas_bert"] = os.path.join(paths["data_path"], "qafeat")
paths["dense_bert"] = os.path.join(paths["data_path"], "dfeat_frame_test")
paths["sub_bert"] = os.path.join(paths["data_path"], "sfeat")
paths["vfeat"] = os.path.join(paths["data_path"], "vfeatdata_test")
paths["csfeat"] = os.path.join(paths["data_path"], "csfeatdata_test")
paths["pretrained_model"] = "./best_release_7420.pth"
...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        self.parser.add_argument("--device", type=int, default=-2, help="0 cuda, -1 cpu")
        ...
        # are we using common-sense features?
        self.parser.add_argument("--cs", action='store_true', default=False,
                                 help="are we using common-sense features?")
        self.parser.add_argument("--mode", type=str, default="test")

    def add_path_cfg(self):
        opt = self.opt

        opt.train_path = os.path.join(paths["raw_data_path"], "tvqa_train_processed.json")
        opt.valid_path = os.path.join(paths["raw_data_path"], "tvqa_val_processed.json")
        opt.test_path = os.path.join(paths["raw_data_path"], "tvqa_test_public_processed.json")
        

        base_path = "" 
        opt.glove_path = os.path.join(paths["raw_data_path"], "glove.6B.300d.txt")
        opt.word2idx_path = os.path.join(paths["raw_data_path"], "cache", "word2idx.pickle")
        opt.idx2word_path = os.path.join(paths["raw_data_path"], "cache", "idx2word.pickle")
        opt.vocab_embedding_path = os.path.join(paths["raw_data_path"], "cache", "vocab_embedding.pickle")
