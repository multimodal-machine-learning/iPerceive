...
def main(cfg):
    ...
    train_dataset = ActivityNetCaptionsIteratorDataset(
        ...
        cfg.use_categories, props_are_gt=True, get_full_feat=False, cs=cfg.cs
    )
    ...
    val_pred_prop_dataset = ActivityNetCaptionsIteratorDataset(
        ...
        cfg.val_2_meta_path, torch.device(cfg.device), 'val_1', cfg.modality, 
        cfg.use_categories, props_are_gt=False, get_full_feat=False, cs=cfg.cs
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cs", action='store_true', default=False, help="are we using common-sense features?")
